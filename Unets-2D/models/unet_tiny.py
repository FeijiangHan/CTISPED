...
class UNet3tiny(nn.Module):
    """
    3 level UNet architecture with tiny convolution blocks that 
    keep feature map sizes unchanged.
    """
    
    def __init__(self, num_classes, in_channels, 
                 is_attention=False, is_recurrent_residual=False):
        ...
        # Standard max pool 2x downsampling 
        self.down = nn.MaxPool2d(kernel_size=2, stride=2)  
        
        # Standard bilinear upsampling 2x
    ...
    def forward(self, x):
        ...
        if self.is_attention:
            x2_2 = self.conv2_2(torch.cat([self.attention2_2(x2_0, x3_0), self.up(x3_0)], dim=1))
            ...
            x2_2 = self.conv2_2(torch.cat([x2_0, self.up(x3_0)], dim=1))
...
class UNet2tiny(nn.Module):
    """
    2 level UNet architecture with tiny convolution blocks.
    """
    ...
    def forward(self, input):
        ...
        if self.is_attention:
            ...
            x1_3 = self.conv1_3(torch.cat([x1_0, self.up(x2_0)], dim=1))
            x0_4 = self.conv0_4(torch.cat([x0_0, self.up(x1_3)], dim=1))
...
class UNet1tiny(nn.Module):
    """
    1 level UNet architecture with tiny convolution blocks. 
    """
    ...
    def forward(self, input):
        ...
        if self.is_attention:
            ...
            x0_4 = self.conv0_4(torch.cat([x0_0, self.up(x1_0)], dim=1))

        # 计算每个类别上的得分, x0_4:[16, 32, 128, 128]
        output = self.final(x0_4)

        return output   # [16, 3, 128, 128]
